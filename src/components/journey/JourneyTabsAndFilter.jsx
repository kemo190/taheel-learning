...
export default function JourneyTabsAndFilter({ dict, locale }) {
  ...
  return (
    <div
      className="flex flex-col xl:flex-row items-center gap-4 mb-8 w-full"
      dir={isRtl ? "rtl" : "ltr"}
    >
      {/* Category Dropdown */}
      <div className="relative z-50 w-full xl:w-auto">
        ...
        >
          ...
          <div className="flex w-full items-center justify-between truncate dark:text-white">
            <span style={{ pointerEvents: "none" }}>
              <span
                className={`truncate ${selectedCategory ? "text-[#0b2646] font-medium" : "text-gray-500 font-medium"}`}
              >
                {selectedCategory ||
                  dict?.journey?.filters?.category ||
                  "تصنيف"}
              </span>
            </span>
          </div>
          <svg
            ...
            className={`lucide lucide-chevron-down text-[#0b2646] size-6 transition-transform duration-200 ${isCategoryOpen ? "rotate-180" : ""}`}
            aria-hidden="true"
          >
        ...
        {/* Dropdown Menu */}
        <div
          className={`absolute top-full ${isRtl ? "right-0" : "left-0"} mt-2 w-full min-w-[200px] bg-white rounded-xl shadow-lg border border-gray-100 z-50 overflow-hidden py-2 transition-all duration-200 origin-top ${isCategoryOpen ? "opacity-100 scale-y-100 visible" : "opacity-0 scale-y-95 invisible"}`}
        >
          {[
            dict?.journey?.filters?.categories?.all || "الكل",
            dict?.journey?.filters?.categories?.oldest || "من الاقدم للاحدث",
            dict?.journey?.filters?.categories?.newest ||
            ...
            <button
              ...
              className={`w-full text-start px-4 py-2.5 text-sm md:text-base transition-colors ${
      ...
      <div className="w-full xl:w-auto xl:ms-auto flex justify-center">
        <div className="flex w-full xl:w-fit items-center rounded-2xl border border-[#DEDEDE]">
          <button
            ...
            className={`relative inline-flex items-center justify-center gap-2 whitespace-nowrap duration-300 cursor-pointer text-base transition-colors focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 h-[3.125rem] px-[1.5rem] py-3 w-1/2 xl:w-48 rounded-2xl font-normal [&_svg]:size-6 ${
              activeTab === "courses"
