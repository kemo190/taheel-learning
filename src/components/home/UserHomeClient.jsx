...
import { dummyInProgressCourses, dummyFavoriteCourses, dummyAICourses, dummyMarketingCourses } from '@/data/dummyCourses';
...
export default function UserHomeClient({ dict, locale, user, profile }) {
  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto pb-16 pt-6 px-4 md:px-0" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      >
        <div className="flex md:basis-[547px] flex-col gap-4 relative z-10 p-6 md:p-8 text-center items-center justify-center">
          ...
          <Link href={`/${locale}/courses`} className="relative inline-flex w-full items-center justify-center gap-2 whitespace-nowrap duration-300 cursor-pointer rounded-2xl text-lg font-bold transition-colors bg-white text-[#0b2646] hover:bg-gray-100 h-[3.125rem] px-[1.5rem] py-3 mt-4">
      ...
      <section className="mx-auto mt-16 w-full max-w-[96%] xl:max-w-[1400px] space-y-6">
        <h4 className="text-[32px] text-[#0b2646] font-bold text-start" dir={isRtl ? 'rtl' : 'ltr'}>
        ...
        <div className="flex justify-start w-full mt-6" dir={isRtl ? 'rtl' : 'ltr'}>
          <Link href={`/${locale}/courses`} className="inline-flex items-center gap-2 text-[#0b2646] font-bold text-[15px] hover:underline transition-all">
