...
export default function LoginForm({ dict, isRtl, locale }) {
  ...
  const schema = z.object({
    ...
    password: z.string().min(6, { message: locale === 'ar' ? 'كلمة المرور غير صحيحة' : 'Invalid password' }),
  });
  ...
  const onSubmit = async (formData) => {
    ...
    if (error) {
      if (error.message.includes('Email not confirmed')) {
        ...
        setServerError(locale === 'ar' 
          ? 'حسابك غير مفعل. لقد قمنا بإرسال رابط تفعيل جديد إلى بريدك الإلكتروني.' 
          : 'Account not verified. A new verification link has been sent to your email.');
      } else {
        setServerError(locale === 'ar' && error.message.includes('Invalid login') 
    ...
    <div className="relative z-30 mt-6 flex w-full flex-col gap-4 px-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
        ...
        <div className="space-y-2">
          <label className="block text-sm font-bold text-[#4b5563] rtl:text-right ltr:text-left">
            {dict.auth.email}
          </label>
