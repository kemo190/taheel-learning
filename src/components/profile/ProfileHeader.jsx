  ...
  const handleImageUpload = async (event) => {
    try {
      ...
      if (!event.target.files || event.target.files.length === 0) {
        throw new Error(
          dict?.profile?.header?.mustSelectImage ||
      ...
      toast.success(
        dict?.profile?.header?.imageUpdated ||
      ...
      <div className="relative px-6 md:px-10 pb-5 pt-2 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-end gap-5">
          ...
          <div className="relative shrink-0 z-10 -mt-12 md:-mt-16">
            {/* Outer ring */}
            <div
              className={`w-28 h-28 md:w-36 md:h-36 rounded-full p-1 bg-white shadow-sm ${isUploading ? "opacity-60" : ""}`}
            >
            ...
            <div className="absolute bottom-1 rtl:left-1 ltr:right-1 z-20">
              {isUploading ? (
                <div className="bg-white p-2 rounded-full shadow-md border border-gray-200 flex items-center justify-center">
                  <svg
                    className="animate-spin h-4 w-4 text-[#0b2646]"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
                ...
                <>
                  ...
                  >
                    ...
                    <span className="sr-only">
                      {dict?.profile?.header?.uploadNewPhoto ||
                        "Upload new photo"}
                    </span>
                  </label>
                  <input
          ...
          <div className="flex flex-col items-center md:items-start text-center md:text-start pb-2">
            ...
            <p className="text-gray-500 text-sm md:text-base font-medium">
              {dict?.profile?.hello || (isRtl ? "أهلاً" : "Hello")},{" "}
              {dict?.profile?.readyToContinue ||
                (isRtl
                  ? "جاهز تكمل رحلتك انهاردة"
