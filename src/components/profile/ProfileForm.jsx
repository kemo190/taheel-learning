...
export default function ProfileForm({
  ...
  const workFields = [
    {
      id: "marketing",
      label: dict?.profile?.form?.workFields?.marketing || "Marketing",
    },
    {
      id: "operations",
      label:
        dict?.profile?.form?.workFields?.operations ||
        "Operations and Supply Chain",
    },
    {
      id: "selfDev",
      label: dict?.profile?.form?.workFields?.selfDev || "Self Development",
    },
    {
      id: "productivity",
      label: dict?.profile?.form?.workFields?.productivity || "Productivity",
    },
    {
      id: "teaching",
      label:
        dict?.profile?.form?.workFields?.teaching || "Teaching and Training",
    },
    {
      id: "finance",
      label:
        dict?.profile?.form?.workFields?.finance || "Finance and Accounting",
    },
    {
      id: "hr",
      label: dict?.profile?.form?.workFields?.hr || "HR and People Management",
    },
    {
      id: "sales",
      label:
        dict?.profile?.form?.workFields?.sales ||
        "Sales and Business Development",
    },
    {
      id: "software",
      label:
        dict?.profile?.form?.workFields?.software || "Software Development",
    },
    {
      id: "business",
      label:
        dict?.profile?.form?.workFields?.business ||
        "Business and Entrepreneurship",
    },
    {
      id: "design",
      label:
        dict?.profile?.form?.workFields?.design || "Design and Digital Arts",
    },
    {
      id: "ai",
      label: dict?.profile?.form?.workFields?.ai || "AI and Data Science",
    },
  ...
  const validate = () => {
    ...
    if (!formData.nameEn)
      newErrors.nameEn =
        dict?.profile?.form?.errors?.englishNameReq ||
        "English name is required";
    if (!formData.nameAr)
      newErrors.nameAr =
        dict?.profile?.form?.errors?.arabicNameReq || "Arabic name is required";
    if (!formData.certificateName)
      newErrors.certificateName =
        dict?.profile?.form?.errors?.certificateNameReq ||
        "Certificate name is required";
    if (!formData.gender)
      newErrors.gender =
        dict?.profile?.form?.errors?.genderReq || "Gender is required";
    if (!formData.dob)
      newErrors.dob =
        dict?.profile?.form?.errors?.dobReq || "Date of birth is required";
    if (!formData.country)
      newErrors.country =
        dict?.profile?.form?.errors?.countryReq || "Country is required";
    if (!formData.governorate && states.length > 0)
      newErrors.governorate =
        dict?.profile?.form?.errors?.governorateReq ||
        "Governorate is required";
    if (!formData.phone || !isValidPhoneNumber(formData.phone))
      newErrors.phone =
        dict?.profile?.form?.errors?.phoneInvalid || "Invalid phone number";
    if (!formData.educationStatus)
      newErrors.educationStatus =
        dict?.profile?.form?.errors?.educationReq ||
        "Education status is required";
    if (!formData.workField)
      newErrors.workField =
        dict?.profile?.form?.errors?.workFieldReq || "Work field is required";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5 pt-1">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            placeholder={
              dict?.profile?.form?.writeNameHere || "Write your name here"
            }
        ...
        <div className="flex flex-col gap-1.5">
          <label
            className="text-[#0b2646] font-bold text-sm"
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex gap-4 h-[40px]">
            <label
              className={`flex-1 flex items-center justify-center gap-3 px-4 bg-gray-50 border rounded-xl cursor-pointer transition-all ${formData.gender === "male" ? "border-[#0b2646] ring-1 ring-[#0b2646]" : errors.gender ? "border-red-500" : "border-gray-200"}`}
            >
              <span className="text-gray-700 text-sm font-medium">
                {dict?.profile?.form?.male || "Male"}
              </span>
              <input
                type="radio"
                name="gender"
            ...
            <label
              className={`flex-1 flex items-center justify-center gap-3 px-4 bg-gray-50 border rounded-xl cursor-pointer transition-all ${formData.gender === "female" ? "border-[#0b2646] ring-1 ring-[#0b2646]" : errors.gender ? "border-red-500" : "border-gray-200"}`}
            >
              <span className="text-gray-700 text-sm font-medium">
                {dict?.profile?.form?.female || "Female"}
              </span>
              <input
                type="radio"
                name="gender"
          ...
          <div className="relative">
            ...
            >
              <option value="" disabled>
                {dict?.profile?.form?.selectCountry || "Select Country"}
              </option>
              {countries.map((c) => (
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="relative">
            ...
            >
              <option value="" disabled>
                {dict?.profile?.form?.selectGovernorate || "Select Governorate"}
              </option>
              {states.map((s) => (
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div
            dir="ltr"
            className={`w-full bg-gray-50 rounded-xl px-4 border focus-within:ring-1 focus-within:ring-[#0b2646] focus-within:border-[#0b2646] transition-all ${errors.phone ? "border-red-500" : "border-gray-200"}`}
          >
            <PhoneInput
              international
              defaultCountry={
                formData.country && isSupportedCountry(formData.country)
                  ? formData.country
                  : "EG"
              }
              ...
              numberInputProps={{
                className:
                  "flex-1 w-full h-full bg-transparent border-none outline-none text-gray-700 text-sm focus:ring-0",
              }}
            />
          </div>
          {errors.phone && (
          ...
          <div className="relative">
            ...
            >
              ...
              <option value="graduated">
                {dict?.profile?.form?.graduated || "Graduated"}
              </option>
              <option value="student">
                {dict?.profile?.form?.student || "Student"}
              </option>
            </select>
            <div className="absolute top-1/2 -translate-y-1/2 ltr:right-4 rtl:left-4 text-gray-400 pointer-events-none">
              <svg
