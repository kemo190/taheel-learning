  ...
  if (user) {
    return (
      <div className="flex items-center gap-3 sm:gap-5">
        ...
        <Link
          href={`/${locale}/journey`}
          className="hidden lg:block text-[#5c6b81] hover:text-[#0b2646] text-[15px] font-medium transition-colors"
        >
        ...
        <div className="relative group pt-1">
          ...
          <div
            className={`absolute top-full rtl:left-0 ltr:right-0 mt-3 w-[260px] bg-white rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] transition-all duration-200 z-50 overflow-hidden border border-gray-100 ${isDropdownOpen ? "opacity-100 visible" : "opacity-0 invisible lg:group-hover:opacity-100 lg:group-hover:visible lg:group-focus-within:opacity-100 lg:group-focus-within:visible"}`}
          >
            {/* Header linked to Profile */}
            <Link
              href={`/${locale}/profile`}
              onClick={() => setIsDropdownOpen(false)}
              className="p-4 border-b border-gray-100 flex items-center gap-3 hover:bg-gray-50 transition-colors cursor-pointer"
            >
              <div
                className={`w-10 h-10 shrink-0 ${avatarUrl ? "bg-gray-100" : "bg-[#0b2646] text-white"} rounded-full flex items-center justify-center overflow-hidden border border-gray-200`}
              >
                {avatarUrl ? (
                  <Image
                    src={avatarUrl}
            ...
            <div className="py-2">
              <Link
                href={`/${locale}/journey`}
                onClick={() => setIsDropdownOpen(false)}
                className="flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-[#0b2646] hover:bg-blue-50 transition-colors"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="18"
              ...
              <Link
                href={`/${locale}/journey?tab=favorites`}
                onClick={() => setIsDropdownOpen(false)}
                className="flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-[#0b2646] hover:bg-blue-50 transition-colors"
              >
                <svg
              ...
              <Link
                href={`/${locale}/journey?tab=certificates`}
                onClick={() => setIsDropdownOpen(false)}
                className="flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-[#0b2646] hover:bg-blue-50 transition-colors"
              >
                <svg
        ...
        <Link
          ...
          className="relative p-1 text-[#8fa7e6] hover:text-[#0b2646] transition-colors flex items-center justify-center"
          aria-label="Wishlist"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
